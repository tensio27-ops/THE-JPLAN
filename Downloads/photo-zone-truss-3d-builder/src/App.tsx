...
<<<<<<< HEAD
      ...
      {carpetColor && (
        <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.015, 0]} receiveShadow>
          ...
          <meshStandardMaterial color={carpetColor} roughness={1} />
        </mesh>
      )}

      {/* Realistic Grass Floor */}
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.02, 0]} receiveShadow>
        ...
        <meshStandardMaterial
          ...
          metalness={0}
          map={useMemo(() => {
            // Using a high-quality seamless grass texture
            const tex = new THREE.TextureLoader().load('https://raw.githubusercontent.com/mrdoob/three.js/master/examples/textures/terrain/grasslight-big.jpg');
...
>>>>>>> f11e4add028f6bab7dd84908080dbfb3a70d8d92
    </group>
  );
}
...
<<<<<<< HEAD
  ...
  const [carpetColor, setCarpetColor] = useState<string | null>(null);
  const [designImage, setDesignImage] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<'size' | 'parts' | 'design' | 'inventory' | 'blueprint'>('size');
  const [cameraView, setCameraView] = useState<'front' | 'top' | 'side' | 'custom'>('custom');
=======
  const [designImage, setDesignImage] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<'size' | 'parts' | 'design' | 'inventory' | 'blueprint'>('size');
>>>>>>> f11e4add028f6bab7dd84908080dbfb3a70d8d92
...
<<<<<<< HEAD
              className={`flex-none px-4 py-3 text-[11px] font-bold transition-colors relative ${activeTab === tab ? 'text-emerald-600' : 'text-stone-500 hover:text-stone-700'
                }`}
=======
...
<<<<<<< HEAD
                    <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${isBuildable ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'
                      }`}>
=======
...
<<<<<<< HEAD
                        className={`text-[11px] py-2 rounded-lg border transition-all ${width === p.w && height === p.h
            ...
            {activeTab === 'design' && (
              ...
              >
                <div className="space-y-4">
                  <label className="block text-sm font-semibold text-stone-700 flex items-center gap-2">
<<<<<<< HEAD
                    <Box className="w-4 h-4" /> 트러스 타입
                  </label>
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      onClick={() => setTrussType('square')}
                      className={`py-2 rounded-lg text-xs font-bold border transition-all ${trussType === 'square' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-stone-600 border-stone-200'
                        }`}
                    >
                      사각 트러스
                    </button>
                    <button
                      ...
                      className={`py-2 rounded-lg text-xs font-bold border transition-all ${trussType === 'triangle' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-stone-600 border-stone-200'
                        }`}
                    >
                      삼각 트러스
                    </button>
                ...
                <div className="space-y-4">
                  <label className="block text-sm font-semibold text-stone-700 flex items-center gap-2">
                    <Layers className="w-4 h-4" /> 바닥 카펫 설정
                  </label>
                  <div className="grid grid-cols-4 gap-2">
                    {[
                      { name: '없음', color: null },
                      { name: '레드', color: '#b91c1c' },
                      { name: '회색', color: '#4b5563' },
                      { name: '청색', color: '#1d4ed8' },
                      { name: '검정', color: '#111827' },
                      { name: '녹색', color: '#15803d' },
                      { name: '백색', color: '#f9fafb' },
                    ].map((c) => (
                      <button
                        key={c.name}
                        onClick={() => setCarpetColor(c.color)}
                        className={`py-2 rounded-lg text-[10px] font-bold border transition-all ${carpetColor === c.color ? 'bg-stone-800 text-white border-stone-800' : 'bg-white text-stone-600 border-stone-200'
                          }`}
                      >
                        {c.name}
                      </button>
...
<<<<<<< HEAD
      {/* 3D Viewport */}
      <main className="flex-1 relative bg-sky-200">
        <Canvas
...
<<<<<<< HEAD
          ...
          <CameraRig view={cameraView} height={height} controlsRef={orbitControlsRef} />
